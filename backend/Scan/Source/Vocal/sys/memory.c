...
/*****************************************************************************/
#pragma DATA_SECTION (vsys_memory,".vsysmem");
...
int vsys_memory[MEMORY_SIZE];

#define sys_free	vsys_free
#define sys_base	vsys_base
#define memsize		vmemsize
#define first_call	vfirst_call
...
void minit (void)
{
    ...
    if (((memsz_t) vsys_memory ^ OVERHEAD) & 1)
    {
		sys_free = (PACKET *) (vsys_memory + 1);
		-- memsize;
    }
    else
    {
		sys_free = (PACKET *) vsys_memory;
    }
...
uint16 alloc_buffer[MAX_LOG_SIZE];
uint16 alloc_buffer_pos = 0;

void log_alloc(int type, void *userptr) {
	uint32 spam;

	spam = (uint32)userptr;
	if (type) {
		spam = spam | 0x80000000;
	}

	alloc_buffer[alloc_buffer_pos++] = (spam >> 16);
	alloc_buffer[alloc_buffer_pos++] =  spam;
	alloc_buffer[alloc_buffer_pos++] =  ((uint32)Curproc->name) >> 16;
	alloc_buffer[alloc_buffer_pos++] =   (uint32)Curproc->name;

	if (alloc_buffer_pos >= MAX_LOG_SIZE - 4) alloc_buffer_pos = 0;
}
#endif /* LOG_ALLOCATION */

void vocal_heap_reset (void)
{
	first_call = 1;

	memset(vsys_memory, 0, MEMORY_SIZE);
}
...
void *malloc (size_t size)
{
	...
	if (- current->packet_size > (allocsize + OVERHEAD + MINSIZE))
	{
		...
		next = (PACKET *) ((long) current + allocsize + OVERHEAD);
...
void free (void *userptr)
{
	...
	next = sys_free;
	prev = 0;
	sysblock = (PACKET *) ((long) userptr - OVERHEAD);
	...
	if ((long) sysblock + sysblock->packet_size + OVERHEAD == (long) next)
	{
	...
	if (prev)				/* ARE WE THE NEW HEAD OF THE LIST */
	{
		...
		/* prev								     */
		if ((long) prev - prev->packet_size + OVERHEAD == (long) sysblock)
		{
...
void *realloc (void *userptr, size_t size)
{
	...
	next = (PACKET *) ((long) userptr + sysblock->packet_size);
	...
	if ((newsize > sysblock->packet_size) &&
		  (((long) next >= (long) sys_base + memsize) ||
		   (next->packet_size > 0)                      ||
		   ((next->packet_size < 0) &&
	...
	if ((long) next == (long) userptr + sysblock->packet_size)
	{
		...
		else
		{
			...
			PACKET *newnext = (PACKET*) ((long) userptr + newsize);
...
int xxyyzz_trace[100];
int xxyyzz_code = 0;

void nsp_xxyyzz (int code)
{
}
void xxyyzz (int code)
{
}
