 ...
 */
ENTRY void
tcp_input (struct iface *iface, struct ip *ip, struct mbuf *bp, int rxbroadcast)
{
	struct tcb *ntcb;
	...
	conn.local = ip->dest;
	conn.local.sin_port = seg.dest;
	conn.remote = ip->source;
	conn.remote.sin_port = seg.source;
	...
	if ((tcb = lookup_tcb (&conn)) == NULLTCB) {
		...
		tcb->conn.local = ip->dest;
		tcb->conn.local.sin_port = seg.dest;
		tcb->conn.remote = ip->source;
		tcb->conn.remote.sin_port = seg.source;
	}
	...
	for (;;) {
		...
		while (tcb->reseq != NULLRESEQ && seq_ge (tcb->rcv.nxt, tcb->reseq->seg.seq)) {
			get_reseq (tcb, (char *) &ip->tos, &seg, &bp, &length);
...
ENTRY void
tcp_icmp (sint31 icsource, sint31 source, sint31 dest, char type, char code, struct mbuf **bpp)
{
	struct tcp seg;
	 ...
	 */
	ntohtcp (&seg, bpp);
	conn.local.sin_port = seg.source;
	conn.remote.sin_port = seg.dest;
	conn.local.sin_address = source;
	conn.remote.sin_address = dest;
	if ((tcb = lookup_tcb (&conn)) == NULLTCB) {
 ...
 */
ENTRY void
reset (struct ip *ip, register struct tcp *seg)
{
	struct mbuf *hbp;
	...
	ph.source = ip->dest.sin_address;
	ph.dest = ip->source.sin_address;
 ...
 */
LOCAL void
update (register struct tcb *tcb, register struct tcp *seg, uint16 length)
{
	uint16 acked;
	uint16 expand;
	...
	if (tcb->cwind < tcb->snd.wnd && ! tcb->flags.retran) {
		if (tcb->cwind < tcb->ssthresh) {
			...
			expand = ((sint31) tcb->mss * tcb->mss) / tcb->cwind;
		}
		/* Guard against arithmetic overflow */
		if (tcb->cwind + expand < tcb->cwind)
			expand = MAX_UINT16 - tcb->cwind;
	...
	if (tcb->flags.rtt_run && seq_ge (seg->ack, tcb->rttseq)) {
		...
		tcb->flags.rtt_run = 0;
		if (!(tcb->flags.retran)) {
			sint31 rtt;	/* measured round trip time */
			sint31 abserr;	/* abs(rtt - srtt) */
 ...
 */
LOCAL int
in_window (struct tcb *tcb, sint31 seq)
{
	return seq_within (seq, tcb->rcv.nxt, (sint31) (tcb->rcv.nxt + tcb->rcv.wnd -1));
}
...
proc_syn (register struct tcb *tcb, char tos, struct tcp *seg)
{
	 ...
	 */
	if ((mtu = rt_mtu (&tcb->conn.remote)) != 0) {
...
LOCAL void
add_reseq (struct tcb *tcb, char tos, struct tcp *seg, struct mbuf *bp, uint16 length)
{
...
LOCAL void
get_reseq (register struct tcb *tcb, char *tos, struct tcp *seg, struct mbuf **bp, uint16 *length)
{
 ...
 */
LOCAL int
trim (register struct tcb *tcb, register struct tcp *seg, struct mbuf **bpp, uint16 *length)
{
	sint31 dupcnt, excess;
	...
	if (tcb->rcv.wnd == 0) {
		...
		if (in_window (tcb, seg->seq)) {
			...
			if (in_window(tcb,(sint31)(seg->seq+len-1)) || /* End is */
			 seq_within(tcb->rcv.nxt,seg->seq,(sint31)(seg->seq+len-1))) { /* Straddles */
				accept++;
			}
		}
	}
